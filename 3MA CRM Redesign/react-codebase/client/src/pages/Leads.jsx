...
// Priority + stage colours — 3MA brand palette.
const priorityColor = {
  Low:    'var(--color-navy)',
                ...
                <tbody>
                  {leads.map(l => (
                    <tr key={l.id}>
                      ...
                      <td style={S.td}>
                        <select value={l.stage} onChange={e => updateStage(l.id, e.target.value)} style={{ ...S.select, padding: '4px 10px', fontFamily: 'var(--font-heading)', fontSize: '0.68rem', fontWeight: 800, letterSpacing: '0.08em', textTransform: 'uppercase', background: stageColor[l.stage] || 'var(--color-gold)', color: l.stage === 'Qualified' || l.stage === 'Proposal' ? 'var(--color-navy)' : '#fff', border: 'none', borderRadius: 999, cursor: 'pointer' }}>
